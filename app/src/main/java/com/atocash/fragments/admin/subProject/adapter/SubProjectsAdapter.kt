...
class SubProjectsAdapter(
    private val items: ArrayList<SubProjectsResponse>,
    ...
    interface CostCenterCallback {
        fun onEdit(item: SubProjectsResponse)
        fun onDelete(item: SubProjectsResponse)
    }
    ...
    override fun onCreateViewHolder(
        ...
        when (viewType) {
            ...
            Keys.RecyclerItem.ITEM -> baseVH = ItemVh(
                ItemSubProjectsBinding.inflate(
                    LayoutInflater.from(
                        parent.context
                    ), parent, false
                )
                ...
                baseVH = EmptyVH(
                    ...
                    ), parent.context.getString(R.string.no_sub_projects_avail)
                )
            }
        }
        return baseVH
    ...
    inner class ItemVh(private val binding: ItemSubProjectsBinding) : BaseVH(binding.root) {
        override fun onBind(position: Int) {
            val itemVm = SubProjectsVhVm(items[position])
    ...
    fun add(item: SubProjectsResponse) {
        items.add(item)
        notifyItemInserted(items.size - 1)
    }

    fun addAll(newItems: ArrayList<SubProjectsResponse>) {
        items.clear()
    ...
    fun remove(item: SubProjectsResponse) {
    ...
    private fun getItem(position: Int): SubProjectsResponse {
