...
class TasksAdapter(
    private val items: ArrayList<TasksResponse>,
    ...
    interface CostCenterCallback {
        fun onEdit(item: TasksResponse)
        fun onDelete(item: TasksResponse)
    }
        ...
        when (viewType) {
            ...
            Keys.RecyclerItem.ITEM -> baseVH = ItemVh(
                ...
                baseVH = EmptyVH(
                    ...
                    ), parent.context.getString(R.string.no_tasks_available)
                )
            }
        }
        return baseVH
    ...
    inner class ItemVh(private val binding: ItemTasksBinding) : BaseVH(binding.root) {
        override fun onBind(position: Int) {
            val itemVm = TasksVhVm(items[position])
    ...
    fun add(item: TasksResponse) {
        items.add(item)
        notifyItemInserted(items.size - 1)
    }

    fun addAll(newItems: ArrayList<TasksResponse>) {
        items.clear()
    ...
    private fun getItem(position: Int): TasksResponse {
