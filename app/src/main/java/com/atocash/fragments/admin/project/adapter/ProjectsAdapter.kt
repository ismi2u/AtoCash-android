...
class ProjectsAdapter(
    private val items: ArrayList<ProjectsResponse>,
    ...
    interface CostCenterCallback {
        fun onEdit(item: ProjectsResponse)
        fun onDelete(item: ProjectsResponse)
    }
        ...
        when (viewType) {
            ...
            Keys.RecyclerItem.ITEM -> baseVH = ItemVh(
                ...
                baseVH = EmptyVH(
                    ...
                    ), parent.context.getString(R.string.no_projects_avail)
                )
            }
        }
        return baseVH
    ...
    inner class ItemVh(private val binding: ItemProjectsBinding) : BaseVH(binding.root) {
        override fun onBind(position: Int) {
            val itemVm = ProjectsVhVm(items[position])
    ...
    fun add(item: ProjectsResponse) {
        items.add(item)
        notifyItemInserted(items.size - 1)
    }

    fun addAll(newItems: ArrayList<ProjectsResponse>) {
        items.clear()
    ...
    fun remove(item: ProjectsResponse) {
    ...
    private fun getItem(position: Int): ProjectsResponse {
