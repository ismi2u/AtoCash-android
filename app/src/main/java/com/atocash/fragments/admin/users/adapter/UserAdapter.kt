...
import com.atocash.databinding.ItemUsersBinding
import com.atocash.network.response.UsersResponse
import com.atocash.utils.Keys

class UserAdapter(
    private val items: ArrayList<UsersResponse>,
    ...
    interface CostCenterCallback {
        fun onEdit(item: UsersResponse)
        fun onDelete(item: UsersResponse)
    }
        ...
        when (viewType) {
            ...
            Keys.RecyclerItem.ITEM -> baseVH = ItemVh(
                ...
                baseVH = EmptyVH(
                    ...
                    ), parent.context.getString(R.string.no_users_avail)
                )
            }
        }
        return baseVH
    ...
    inner class ItemVh(private val binding: ItemUsersBinding) : BaseVH(binding.root) {
        override fun onBind(position: Int) {
            val itemVm = UsersVhVm(items[position])
    ...
    fun add(item: UsersResponse) {
        items.add(item)
        notifyItemInserted(items.size - 1)
    }

    fun addAll(newItems: ArrayList<UsersResponse>) {
        items.clear()
