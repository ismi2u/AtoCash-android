...
import com.atocash.databinding.ItemCurrencyBinding
...
class CurrencyAdapter(
    private val items: ArrayList<CurrencyResponse>,
    ...
    interface CostCenterCallback {
        fun onEdit(item: CurrencyResponse)
        fun onDelete(item: CurrencyResponse)
    }
    ...
    inner class ItemVh(private val binding: ItemCurrencyBinding) : BaseVH(binding.root) {
        override fun onBind(position: Int) {
            val itemVm = CurrencyVhVm(items[position])
    ...
    fun add(item: CurrencyResponse) {
        items.add(item)
        notifyItemInserted(items.size - 1)
    }

    fun addAll(newItems: ArrayList<CurrencyResponse>) {
        items.clear()
    ...
    fun remove(item: CurrencyResponse) {
    ...
    private fun getItem(position: Int): CurrencyResponse {
    ...
    fun removeItem(item: CurrencyResponse) {
