...
class UserRolesAdapter(
    private val items: ArrayList<UserRolesResponse>,
    ...
    interface CostCenterCallback {
        fun onEdit(item: UserRolesResponse)
        fun onDelete(item: UserRolesResponse)
    }
        ...
        when (viewType) {
            ...
            Keys.RecyclerItem.ITEM -> baseVH = ItemVh(
                ...
                baseVH = EmptyVH(
                    ...
                    ), parent.context.getString(R.string.no_user_roles_avail)
                )
            }
        }
        return baseVH
    ...
    inner class ItemVh(private val binding: ItemUserRolesBinding) : BaseVH(binding.root) {
        override fun onBind(position: Int) {
            val itemVm = UserRolesVhVm(items[position])
    ...
    fun add(item: UserRolesResponse) {
        items.add(item)
        notifyItemInserted(items.size - 1)
    }

    fun addAll(newItems: ArrayList<UserRolesResponse>) {
        items.clear()
    ...
    fun remove(item: UserRolesResponse) {
    ...
    private fun getItem(position: Int): UserRolesResponse {
