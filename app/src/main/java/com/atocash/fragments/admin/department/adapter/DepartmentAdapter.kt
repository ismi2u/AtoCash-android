...
import com.atocash.databinding.ItemDepartmentBinding
...
class DepartmentAdapter(
    private val items: ArrayList<DepartmentResponse>,
    ...
    interface CostCenterCallback {
        fun onEdit(item: DepartmentResponse)
        fun onDelete(item: DepartmentResponse)
    }
        ...
        when (viewType) {
            ...
            Keys.RecyclerItem.ITEM -> baseVH = ItemVh(
                ...
                baseVH = EmptyVH(
                    ...
                    ), parent.context.getString(R.string.no_department_avail)
                )
            }
        }
        return baseVH
    ...
    inner class ItemVh(private val binding: ItemDepartmentBinding) : BaseVH(binding.root) {
        override fun onBind(position: Int) {
            val itemVm = DepartmentVhVm(items[position])
    ...
    fun add(item: DepartmentResponse) {
        items.add(item)
        notifyItemInserted(items.size - 1)
    }

    fun addAll(newItems: ArrayList<DepartmentResponse>) {
        items.clear()
    ...
    fun remove(item: DepartmentResponse) {
    ...
    private fun getItem(position: Int): DepartmentResponse {
